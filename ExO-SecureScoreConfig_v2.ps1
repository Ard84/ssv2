...
function connectEXOAsMSI ($OAuthCredential) {
    ...
    If (((@($psSessions) -like '@{State=Opened; Name=RunSpace*').Count -gt 0) -ne $true) {
        ...
        try {
            ...
            $null = Import-PSSession $Session -DisableNameChecking -CommandName "*Get-Msol*","*Get-Transport*","*-Mailbox*","*Get-AutomationVar*","*Get-SafeAttachmentPolicy*","*Get-SafeLinkPolicy*","*New-Transport*","*New-Safe*","*Get-AcceptedDomain*" -AllowClobber
...
$null = Connect-MsolService -Identity
